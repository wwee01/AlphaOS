"""Scheduler v1.5 AI cost guard (PR3; true-up in PR9.5; PR14 adds a nested
daily sub-cap).
...
PR14 adds ``agent_votes`` (the bear-debate agent) as a 5th term here AND a
separate, TIGHTER ``check_debate_budget``/daily sub-cap -- nested INSIDE this
shared cap, not a replacement for it, so a new shadow feature can never
starve the live evaluator's own share of the shared 30-day budget.
"""
...
def calls_in_last_30_days(journal) -> int:
    """Count of real (non-mock) AI calls across all five call sites in the
    trailing 30 days: the primary evaluator, the playbook labeller, the
    narrative-polarity classifier, EVAL-1's offline replay harness, and
    PR14's bear-debate agent -- each a separate real API call that should
    count toward the same spend cap.
    ...
    return evaluations + labels + polarity + eval_replays + debate_votes
