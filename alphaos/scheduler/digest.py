...
def build_daily_digest(journal, settings, kill_switch) -> dict:
    ...
    # PR5: earnings-proximity awareness -- surface pending proposals whose hold
    # window contains an earnings event, shortlisted candidates in the warning
    # window, and provider health (unavailable data / outright failures) so an
    # operator sees both the signal AND whether the signal itself is healthy.
    # Advisory only: nothing here blocks or alters any trade.
