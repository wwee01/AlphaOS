...
- **AI category label is ADVISORY + downgrade-only** (`_apply_label_floor` = `min` over REJECT<WATCH<PROPOSE): it can restrict but never create a PROPOSE, never mint an official label (only the fixed `OFFICIAL_LABELS` set), never overwrite the frozen `primary_label`.
