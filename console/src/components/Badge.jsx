...
// direction/bucket) opt in explicitly. No @keyframes/animation prop --
// §13 calm-console rule, same as the component this replaces.
export function Badge({ tone = 'neutral', caps = false, children, style }) {
  const modifier = TONE_MODIFIER[tone] ?? '';
  const className = ['badge', modifier, caps ? 'badge-caps' : ''].filter(Boolean).join(' ');
